...
btn.addEventListener("click", async function () {
	...
	await fetch(`${dbURL}/results/${projectID}.json`, {
		...
		body: JSON.stringify({
			...
			name: "Малюшевская Ангелина",
		}),
	});
	localStorage.setItem("voted" + projectID, "true");
	btn.classList.add("active-voted");
	document.getElementById("count_heart").innerHTML = currentLikes + 1;
});

// Простой Observer для появления при скролле
